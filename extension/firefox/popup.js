// The agent popup, Firefox build.
//
// Uses the promise-based `browser` API where available, falling back to
// `chrome` so the same file runs unchanged if loaded in a Chromium-based
// Gecko-compatible host.
...
$('form[name="settings"]').addEventListener('change', async () => {
  await send({ kind: 'settings', evalEnabled: $('input[name="evalEnabled"]').checked });
  refresh();
});
